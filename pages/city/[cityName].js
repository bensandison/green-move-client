...
export default function CityData() {
	...
	return (
		<Flex gap={4} maxW="container.md" direction="column" py={4} m="auto">
			<SearchBar value={cityData && cityData.name}></SearchBar>
			<Box>
			...
			<Box>
				...
				<Stack mt="2" direction="row" spacing={1}>
					<MultiLeafScore score={cityData && cityData.rating}></MultiLeafScore>
					<Text fontWeight="semibold" fontSize="3xl" alignSelf="flex-end">
						{cityData && cityData.rating}
					</Text>
				</Stack>
			</Box>
			<Box>
				...
				<SimpleGrid mt="2" columns={[2, 3, 4]} spacing={4}>
					<InfoCard
						title="Air Quality"
						value={cityData && cityData.air_quality}
					></InfoCard>
					<InfoCard
						title="Population"
						value={cityData && cityData.pop}
					></InfoCard>
